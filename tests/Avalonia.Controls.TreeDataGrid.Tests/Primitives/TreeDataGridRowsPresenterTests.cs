...
public sealed class TreeDataGridRowsPresenterTests
{
    ...
    public async Task Handles_Inserted_Row()
    {
        ...
        await AssertSameElements(target.RealizedElements, elements);
    }
    ...
    /// <summary>
    /// Asserts that the presenter's realized elements are the same control instances as
    /// <paramref name="expected"/>, in the same order.
    /// </summary>
    /// <remarks>
    /// These tests are about element recycling, so the elements must be compared by reference:
    /// IsEquivalentTo compares two controls member by member, which walks their whole property
    /// graph and reports differences between distinct-but-similar rows.
    /// </remarks>
    private static async Task AssertSameElements(
