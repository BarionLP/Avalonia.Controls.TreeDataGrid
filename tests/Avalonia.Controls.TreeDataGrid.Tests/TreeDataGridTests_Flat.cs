...
public class TreeDataGridTests_Flat
{
    ...
    public async Task Desired_Width_Should_Be_Total_Of_Fixed_Width_Columns()
    {
        ...
        await Assert.That(target.DesiredSize.Width).IsEqualTo(24);
    }
    ...
    public async Task Should_Size_Star_Columns()
    {
        ...
        foreach (var row in rows)
        {
            var cells = row.CellsPresenter!
                ...
                .ToList();
            await Assert.That(cells.Count).IsEqualTo(2);
            await Assert.That(cells[0].Bounds.Width).IsEqualTo(25);
    ...
    public async Task Should_Size_Star_Columns_With_Max_Width()
    {
        ...
        foreach (var row in rows)
        {
            ...
            await Assert.That(cells[1].Bounds.Width).IsEqualTo(25);
        }
    }
    ...
    public async Task Raises_CellPrepared_Events_On_Initial_Layout()
    {
        ...
        // 10 realized rows of 2 columns each.
        await Assert.That(raised).IsEqualTo(20);
    }
    ...
    public async Task Columns_Are_Correctly_Sized_After_Changing_Source()
    {
        ...
        await Assert.That(columns[1].ActualWidth).IsEqualTo(20);
        await Assert.That(columns[2].ActualWidth).IsEqualTo(20);
    }
    ...
    public async Task Should_Use_TextCell_StringFormat()
    {
        ...
        {
            var cells = rows[i].CellsPresenter!.GetVisualChildren().Cast<TreeDataGridCell>().ToList();
            await Assert.That(cells.Count).IsEqualTo(1);

            var cell = await Assert.That(cells[0]).IsTypeOf<TreeDataGridTextCell>().And.IsNotNull();
