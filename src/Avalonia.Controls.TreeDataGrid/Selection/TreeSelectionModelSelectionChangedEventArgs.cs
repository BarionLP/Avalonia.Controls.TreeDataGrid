...
public class TreeSelectionModelSelectionChangedEventArgs<T>(
    ...
    IReadOnlyList<T?>? selectedItems = null) : TreeSelectionModelSelectionChangedEventArgs
{
    ...
    /// Gets the indexes of the items that were removed from the selection.
    /// </summary>
    public override IReadOnlyList<IndexPath> DeselectedIndexes { get; } = deselectedIndexes ?? [];

    /// <summary>
    /// Gets the indexes of the items that were added to the selection.
    /// </summary>
    public override IReadOnlyList<IndexPath> SelectedIndexes { get; } = selectedIndexes ?? [];

    /// <summary>
    /// Gets the items that were removed from the selection.
    /// </summary>
    public new IReadOnlyList<T?> DeselectedItems { get; } = deselectedItems ?? [];

    /// <summary>
    /// Gets the items that were added to the selection.
    /// </summary>
    public new IReadOnlyList<T?> SelectedItems { get; } = selectedItems ?? [];
    ...
    private class Untyped(IReadOnlyList<T?> source) : IReadOnlyList<object?>
    {
        private readonly IReadOnlyList<T?> _source = source;
