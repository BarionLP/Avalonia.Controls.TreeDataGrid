    ...
    where TIn : class
{
    ...
    public IDisposable Bind(AvaloniaObject target, StyledProperty<TOut> property)
    {
        ...
        return mode switch
        {
            BindingMode.Default or BindingMode.OneWay => target.Bind(property, expression, Priority),
            BindingMode.TwoWay => new CompositeDisposable(
                                target.Bind(property, expression, Priority),
                                target.GetBindingObservable(property).Subscribe(expression)),
            _ => throw new ArgumentException("Invalid binding mode."),
        };
    }
    ...
    public IDisposable Bind(AvaloniaObject target, DirectPropertyBase<TOut> property)
    {
        ...
        return mode switch
        {
            ...
            BindingMode.TwoWay => new CompositeDisposable(
                                target.GetBindingObservable(property).Subscribe(expression),
                                target.Bind(property, expression, Priority)),
            BindingMode.OneTime or BindingMode.OneWayToSource => throw new NotImplementedException(),
            _ => throw new ArgumentException("Invalid binding mode."),
        };
    }
